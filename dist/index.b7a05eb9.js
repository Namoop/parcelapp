...
var _spriteClass = require("./assets/classes/sprite.class");
...
const Time = {
    ...
    ,
    in: async function(time, unit, callback) {
        switch(unit){
            case "m":
            case "minutes":
                time *= 60;
...
cnv.onkeydown = cnv.onkeyup = function(e) {
    if (e.key.length == 1) kp[e.key.toLowerCase() == e.key ? e.key.toUpperCase() : e.key.toLowerCase()] = 0;
...
cnv.onmousemove = (e)=>([windowMouseX, windowMouseY] = [
        e.clientX,
        e.clientY
    ])
;
cnv.ontouchmove = (e)=>([windowMouseX, windowMouseY] = [
        e.touches[0].clientX,
        e.touches[0].clientY, 
    ])
; //tap and place?
...
function setatts(el, vals) {
    for (let i of Object.keys(vals))el.setAttribute(i, vals[i]);
}

},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"6qCP7":[function(require,module,exports) {
module.exports = require('./helpers/bundle-url').getBundleURL('aNMIV') + "bob.866102d2.png" + "?" + Date.now();
